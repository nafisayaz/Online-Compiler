...
io.on('connection',function(socket){
    if((socket.handshake.headers.referer=="http://192.168.1.7:8000/")||( socket.handshake.headers.referer=="http://localhost:8000/")){
        var projectid=uid();
    ...
    */
    ...
    socket.on('compile',function(code,input){
        var path="cache/" + socket.projectID+"/";
        exec("mkdir "+path);
            ...
            fs.writeFile(__dirname+"/"+path+"input.txt",input,function(err){
                exec("g++ "+path+"main.cpp -o "+path+"main.run",function(error,stdout,stderr){
                    ...
                    if(error!=null && error.code!='0')
                    {
                        ...
                        if(!msg)
                            io.in(socket.projectID).emit("compile_msg","<b>***********COMPILATION SUCCESS***************<br></b>");
                        else{
                            msg=replaceAll(msg,path,"");
                            msg=replaceAll(msg,'\n','<br>');
                            io.in(socket.projectID).emit('compile_msg',msg);
